...
// Expect model version (e.g., '2023-01-01' or specific from LogMeal docs) in env
...
async function recognizeWithSegmentation(imagePath) {
  try {
    ...
    if (status === 400 && data?.code === 1302) {
      console.error(`Invalid model version '${LOGMEAL_MODEL_VERSION}'. Please verify LOGMEAL_MODEL_VERSION in .env matches a valid segmentation version from LogMeal docs.`);
      throw new Error(`Segmentation failed: invalid model version '${LOGMEAL_MODEL_VERSION}'. Consult LogMeal documentation for correct version.`);
    }
    console.error('Segmentation recognition error:', data || err.message);
