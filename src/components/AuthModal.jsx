...
const AuthModal = ({ onClose }) => {
  ...
  // Set your backend URL - adjust this to match your backend
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        // After successful Firebase signup, send user data to your backend
        try {
          const idToken = await userCredential.user.getIdToken();
          const { data } = await axios.post(`${API_URL}/api/auth/firebase`, {
            idToken,
            name: name.trim(), // Include name for new users
            isNewUser: true
          }, {
            headers: {
          ...
          // Even if backend fails, Firebase auth succeeded, so we continue
        }
      }
      
      // For login, you might also want to sync with your backend
      if (isLogin) {
        try {
          const idToken = await userCredential.user.getIdToken();
          const { data } = await axios.post(`${API_URL}/api/auth/firebase`, {
