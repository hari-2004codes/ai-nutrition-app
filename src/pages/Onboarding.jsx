...
export default function Onboarding({ onComplete }) {
  ...
  const saveProfileToDatabase = async (profileData) => {
    try {
      ...
      // Try to get the backend token from localStorage
      let token = localStorage.getItem('token');
      console.log('🔍 Backend token from localStorage:', token ? 'exists' : 'missing');
      
      // If no backend token, try to sync with backend using Firebase token
      if (!token) {
        console.log('🔄 No backend token found, syncing with backend...');
        const syncResponse = await fetch('/api/auth/firebase', {
          method: 'POST',
          headers: {
        ...
        if (syncResponse.ok) {
          ...
          throw new Error(`Backend sync failed: ${errorData.msg || 'Unknown error'}`);
        }
      }

      console.log('🔄 Saving profile data:', profileData);

      // Now save the profile with the token
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.msg || `HTTP ${response.status}: Failed to save profile`);
      }
