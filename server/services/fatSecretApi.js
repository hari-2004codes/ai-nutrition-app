...
const API_SECRET = process.env.VITE_FATSECRET_API_SECRET;
const BASE_URL = 'https://platform.fatsecret.com/rest/server.api';

// Helper function to generate OAuth signature using Node.js crypto
const generateOAuthSignature = (params, httpMethod = 'GET', baseUrl = BASE_URL) => {
  ...
  // Sign the message using HMAC-SHA1
...
export const searchFoods = async (query) => {
  ...
  try {
    ...
    // Generate signature
    const signature = generateOAuthSignature(allParams);
    ...
    console.log('Foods search API Response status:', response.status);
...
export const getFoodDetails = async (foodId) => {
  ...
  try {
    ...
    console.log('Food details API Response status:', response.status);
